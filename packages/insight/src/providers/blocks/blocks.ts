...
export class BlocksProvider {
    ...
    public async getAllBlocks() {
        ...
        const dataHeight = await this.httpClient.get<any>('https://sapi.smartcash.cc/v1/blockchain/height').toPromise();
        let currentBlockHeight = dataHeight.height;
        if (cachedBlocks.length === 0) {
            for (let i = 0; i < 10; i++) {
                const block = await this.httpClient.get<ApiEthBlock[] & ApiUtxoCoinBlock[]>(`https://sapi.smartcash.cc/v1/blockchain/block/${currentBlockHeight}`).toPromise<ApiEthBlock[] & ApiUtxoCoinBlock[]>();
    ...
    public getBlock(
        ...
        const url = `https://sapi.smartcash.cc/v1/blockchain/block/${hash}`;
        return this.httpClient.get<ApiEthBlock & ApiUtxoCoinBlock>(url);
    }
}
